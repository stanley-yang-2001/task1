...
export function initMissionSocket(droneController, enuTransform, inverseEnuTransform, homePosition, physics) {
  ...
  window.sendNLCommand = function (command) {
    ...
    console.log("command received in mission_socket: ", command)
  };
  ...
  if (Array.isArray(data.mission)) {
    ...
    if (waypoints.length === 1) {
      console.log("waypoints length is 1")
      droneController.update(waypoints[0]);
    } else {
      //droneController.setWaypoints(waypoints);
      console.log("waypoints length is not 1")
    }

    // Optional: disable manual control
    window.manualInputEnabled = false;
